...
let getDetailedData = (porscheUrl) => {
	return axios.get(porscheUrl).then(response => {
		var specs = $('.specs-list > li', response.data)
			.map(function() { return $(this).text() }).get();
		var detailed_description = $('.trow.clearfix div', response.data)
			.map(function() { return $(this).text() }).get();
		...
		var car = {
			...
			detailed_description: detailed_description[0].replace(/[\n\t]/g, ' ').replace('Description:', '').trim(),
...
module.exports = {
	...
	updateUrl: (url) => {
		...
		// return collection.distinct('url').then(mongoUrls => {
			...
			return getDetailedData(url).then(car => {
				// cars.forEach(car => {
	...
	updateAll: () => {
		return getMaxNumOfRennlistPages()
			...
			return mongoHelper.getMongoCollection('cl_cars', 'porsche').then(collection => {
				// return collection.distinct('url').then(mongoUrls => {
					// var urlsToAdd = rennlistUrls.filter(r => !mongoUrls.includes(r))
					return Promise.all(rennlistUrls.map(u => getDetailedData(u))).then(cars => {
						cars.forEach(car => {
							collection.updateOne({ url: car.url}, {$set: car}, {upsert: true})
						})
						// console.log(cars.flat())
	...
	addMostRecent: () => {
		return getMaxNumOfRennlistPages().catch(err => console.log(err)).then(maxPages => {
			var urls = []
			for (var i = 1; i <= maxPages; i++){
				urls.push(baseUrl + i.toString())
			}
